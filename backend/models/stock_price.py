    ...
    def get_original_price(self) -> list | None:
        try:
            ...
            with stock_connection.cursor(dictionary=True) as cursor:
                ...
                stock_price_query = (f"""SELECT date, open, min, max, close FROM {table} WHERE date between %s and %s""")
                cursor.execute(stock_price_query, (self.start_date, self.end_date))
