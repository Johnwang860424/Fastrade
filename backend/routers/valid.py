...
def validate_date(
    ...
    try:
        ...
        if start_datetime > end_datetime:
            ...
            raise HTTPException(status_code=400, detail="start_date cannot be earlier than 2000-01-01")
        elif end_datetime > datetime.datetime.today():
            raise HTTPException(status_code=400, detail="end_date cannot be later than today")
        elif request.url.path == "/strategy/" and (end_datetime - start_datetime).days < 30:
            raise HTTPException(status_code=400, detail="end_date must be at least 30 days after start_date")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid input parameters")
    return start_date, end_date

def validate_price_type(
   price_type: str = Query(..., regex="^(origin|adjust)$", description="價格類型：\n\norigin：原始價格\n\nadjust：還原價格")
):
    return price_type